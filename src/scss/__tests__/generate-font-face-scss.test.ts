...
describe('Expect generateFontFaceScss', () => {
  ...
  describe('to call generateScssForDir for each font subdirectory', () => {
    it('when input directory contains font subdirectories', () => {
      ...
      expect(generateScssForDir).toHaveBeenCalledWith('/input/dm-sans', '/output/dm-sans', 'dm-sans')
      expect(generateScssForDir).toHaveBeenCalledWith('/input/roboto', '/output/roboto', 'roboto')
    })
  })
