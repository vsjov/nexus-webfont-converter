 ...
 * ```
 */
export const convertFontsInDir = (
  dirPath: string,
  ...
  for (const relPath of fontFiles) {
    ...
    if (formats.includes('woff')) {
      ...
      try {
        ...
        log(pc.red(`Failed to convert ${pc.blue(path.basename(relPath))} to WOFF: ${(err as Error).message}`))
      }
    }
    ...
    if (formats.includes('woff2')) {
      ...
      try {
        ...
        log(pc.red(`Failed to convert ${pc.blue(path.basename(relPath))} to WOFF2: ${(err as Error).message}`))
      }
    }
  }
}
