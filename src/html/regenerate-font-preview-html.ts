 ...
 * ```
 */
export const regenerateFontPreviewHtml = (outputDir: string): void => {
  ...
  for (const { outputFontDir, dirName } of targets) {
    ...
    if (!fs.existsSync(scssPath)) {
      log(pc.yellow(`No SCSS file found in ${pc.blue(outputFontDir)} - skipping HTML regeneration`))
      continue
    }
    ...
    if (fontEntries.length === 0) {
      log(pc.yellow(`No @include fontFace entries found in ${pc.blue(scssPath)} - skipping HTML regeneration`))
      continue
    }
    ...
    log(`Regenerated ${pc.green(`${dirName}.html`)} for ${pc.blue(familyName)} (${fontEntries.length} variants)`)
  }
}
