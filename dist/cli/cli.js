...
export const main = async () => {
    ...
    if (values.native) {
        exitWithError('--native is handled by the wfc wrapper. Invoke wfc rather than dist/cli/cli.js directly.');
    }
    if (values.version) {
        console.log(version);
        process.exit(0);
    ...
    process.stdout.write(`\n${pc.bold(name)} ${pc.dim(`v${version}`)}\n`);
    process.stdout.write(`${pc.dim(description)}\n\n`);
