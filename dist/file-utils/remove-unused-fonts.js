 ...
 * ```
 */
export const removeUnusedFonts = (outputDir) => {
    ...
    for (const { outputFontDir, dirName } of targets) {
        ...
        if (!fs.existsSync(scssPath)) {
            logger.warn(`No SCSS file found in ${pc.blue(outputFontDir)} - skipping`);
            continue;
        }
        const scssContent = fs.readFileSync(scssPath, 'utf-8');
        const entries = parseScssEntries(scssContent);
        if (entries.length === 0) {
            logger.warn(`No parseable fontFace includes found in ${pc.blue(scssPath)} - skipping`);
            continue;
        }
...
//# sourceMappingURL=remove-unused-fonts.js.map