 ...
 */
export const templateHtmlSamples = ({ familyName, dirName, entries, glyphs = [], licenseFile = null, }) => {
    ...
    const variantSections = entries.map(({ weight, style }) => {
        ...
        const fontStyle = `font-family: '${familyName}'; font-weight: ${weight}; font-style: ${style};`;
    ...
    return [
        ...
        `  <title>${familyName} — Font Preview</title>`,
        `  <link rel="stylesheet" href="${dirName}.css">`,
        `  <style>`,
        ...
        ...glyphs.map(key => `    .variant__sample--${toCssClass(key)} { font-size: 1rem; color: #999; }`),
        ...
        ...(licenseFile
            ? [
                `  <footer>`,
                `    <a href="${licenseFile}"><b>License:</b> ${licenseFile}</a>`,
                `  </footer>`,
            ]
            : []),
        `</body>`,
...
//# sourceMappingURL=template-html-samples.js.map