 ...
 */
export const validateReleaseTag = (options) => {
    const versionMatch = RELEASE_TAG_PATTERN.exec(options.tagName);
    if (!versionMatch) {
        throw new Error(`Release tag must use vX.Y.Z without prerelease or leading-zero parts. Received: ${options.tagName}`);
    }
    ...
    if (packageMetadata.name !== PACKAGE_NAME) {
        throw new Error(`Release package must be ${PACKAGE_NAME}. Received: ${packageMetadata.name}`);
    }
    if (packageMetadata.version !== version) {
        throw new Error(`Package version ${packageMetadata.version} does not match release tag ${options.tagName}`);
    }
    ...
    try {
        ...
        throw new Error(`Release tag ${options.tagName} is not available in the checkout`);
    }
    try {
        ...
        throw new Error(`Matching remote release branch origin/release/${version} does not exist`);
    }
    if (tagCommit !== branchCommit) {
        throw new Error(`Release tag ${options.tagName} points at ${tagCommit}, but origin/release/${version} points at ${branchCommit}`);
    }
    ...
    if (releaseTags.length !== 1 || releaseTags[0] !== options.tagName) {
        throw new Error(`Release branch origin/release/${version} must introduce only ${options.tagName} relative to origin/master. Found: ${releaseTags.join(', ') || '(none)'}`);
    }
    return {
        branchRef,
